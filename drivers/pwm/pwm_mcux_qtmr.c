				    ...
				    pwm_flags_t flags)
{
	...
	uint16_t reg;
	uint16_t needed_outmode;
	uint16_t current_outmode;
	bool timer_running;
	bool outmode_change;
	...
	if ((highCount > 0xFFFFU) || (lowCount > 0xFFFFU)) {
		/* This should not be a 16-bit overflow value. If it is, change to a larger
		 * divider for clock source.
		 */
		return -EINVAL;
	}

	/* Determine the output mode required for the requested duty cycle */
	if (pulse_cycles == 0U) {
		needed_outmode = TMR_CTRL_OUTMODE(kQTMR_ClearOnCompare);
	} else if (pulse_cycles == period_cycles) {
		needed_outmode = TMR_CTRL_OUTMODE(kQTMR_SetOnCompare);
	} else {
		needed_outmode = TMR_CTRL_OUTMODE(kQTMR_ToggleOnAltCompareReg);
	}

	k_mutex_lock(&data->lock, K_FOREVER);

	/* Check if the timer is already running by inspecting the count mode bits */
	timer_running = (config->base->CHANNEL[channel].CTRL & TMR_CTRL_CM_MASK) != 0U;

	/* Check if the output mode needs to change (e.g. transitioning to/from 0% or 100%) */
	current_outmode = config->base->CHANNEL[channel].CTRL & TMR_CTRL_OUTMODE_MASK;
	outmode_change = (current_outmode != needed_outmode);

	if (timer_running && !outmode_change) {
		/*
		 * Timer is running and output mode is unchanged — update the preload
		 * registers only. The hardware swaps CMPLD1 -> COMP1 and CMPLD2 -> COMP2
		 * automatically at the next natural compare event, producing a completely
		 * glitch-free update with no interruption to the output.
		 */
		config->base->CHANNEL[channel].CMPLD1 = (uint16_t)lowCount;
		config->base->CHANNEL[channel].CMPLD2 = (uint16_t)highCount;
	} else {
		/*
		 * Either the timer is not running, or the output mode must change
		 * (e.g. transitioning to/from 0% or 100% duty cycle). A full
		 * stop/reconfigure/start is required. This path is taken rarely —
		 * only on first start or on an edge-mode transition — so the
		 * one-time glitch is acceptable.
		 */

		/* Set OFLAG pin for output mode and force out a low on the pin */
		config->base->CHANNEL[channel].SCTRL |=
		 ...
		 * Load COMP1 when CSCTRL[TCF2] is asserted, load COMP2 when CSCTRL[TCF1] is
		 * asserted.
		 */
		...
		reg |= (TMR_CTRL_LENGTH_MASK | needed_outmode);
		config->base->CHANNEL[channel].CTRL = reg;

		/* Clear FORCE now that registers are configured, so the timer can
		 * drive the output pin freely.
		 */
		config->base->CHANNEL[channel].SCTRL &= ~TMR_SCTRL_FORCE_MASK;
...
static void mcux_qtmr_isr(const struct device *dev)
{
	...
	if (data->capture.continuous) {
		if (data->capture.pulse_capture) {
			 ...
			 * first edge of next period capture is second edge of this
			 * capture (this edge)
			 */
		}
	} else {
